...
public class SysDictTypeGatewayImpl extends BaseGatewayImpl implements ISysDictTypeGateway {
    ...
    @Caching(
            evict = {
                    @CacheEvict(cacheNames = CACHE_NAME, key = "#root.target.ALL_PREFIX_COLON+#context.tenantId", cacheManager = CacheConstant.cacheManager),
                    @CacheEvict(cacheNames = CACHE_NAME,key = "#root.target.PREFIX_COLON +#context.tenantId+':'+ #context.type",cacheManager = CacheConstant.cacheManager),
            }
    )
    ...
    @Override
    public SysDictTypeDto findByType(String dictType,Integer tenantId) {
        if (StringUtils.isEmpty(dictType)){
            return null;
        }
        List<SysDictTypeBase> list = sysDictTypeMapper.findListBy(SysDictTypeBase.builder().tenantId(tenantId).type(dictType).build());
    ...
    @Caching(
            evict = {
                    @CacheEvict(cacheNames = CACHE_NAME,key = "#root.target.ALL_PREFIX_COLON +#tenantId",cacheManager = CacheConstant.cacheManager),
                    @CacheEvict(cacheNames = CACHE_NAME,key = "#root.target.PREFIX_COLON +#tenantId+':'+ #dictType",cacheManager = CacheConstant.cacheManager)
            }
    )
    public void deleteCache(String dictType,Integer tenantId) {
    }

    @Override
    @Cacheable(cacheNames = CACHE_NAME, key = "#root.target.ALL_PREFIX_COLON+#tenantId", cacheManager = CacheConstant.cacheManager, unless = "#result == null || #result.isEmpty()")
    public List<SysDictDto> findAllType(Integer tenantId) {
        List<SysDictTypeBase> list = sysDictTypeMapper.findListBy(SysDictTypeBase.builder().tenantId(tenantId).status(StatusEnum.ON.getStatus()).build());
    ...
    @Override
    @CacheEvict(cacheNames = CACHE_NAME,key = "#root.target.ALL_PREFIX_COLON+#tenantId" ,cacheManager = CacheConstant.cacheManager)
    public void deleteAllCache(Integer tenantId) {
